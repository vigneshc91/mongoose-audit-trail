...
function checkRequired(opts, queryObject, updatedObject) {
    ...
    let { __user: user, __reason: reason } = (queryObject && queryObject.options) || updatedObject;
...
function saveDiffObject(currentObject, original, updated, opts, queryObject, method) {
    let { __user: user, __reason: reason, __session: session } = (queryObject && queryObject.options) || currentObject;
 ...
 */
const plugin = function auditTrackPlugin(schema, opts = {}) {
    if (opts.uri) {
    ...
    schema.pre('save', function (next) {
        ...
        this.constructor
            ...
            .then((original) => {
                ...
                return saveDiffObject(this, original ? original : {}, this.toObject({ depopulate: true }), opts, null, method);
            });
